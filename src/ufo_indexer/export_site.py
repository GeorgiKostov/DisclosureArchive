...
<body>
  ...
  <script>
    ...
    const globeState = { ready: false, initializing: false, locations: [], markers: [], selected: null, selectedIndex: null, releaseFilter: null };
    const overlayState = {
    ...
    function applyReleaseFilter() {
      const filter = globeState.releaseFilter;
      globeState.markers.forEach((marker) => {
        const loc = globeState.locations[marker.userData.locationIndex];
        marker.visible = !filter || (loc && loc.release_date === filter);
      });
      if (globeState.selectedIndex != null) {
        const selectedMarker = globeState.markers[globeState.selectedIndex];
    ...
    function buildReleaseControl() {
      ...
      host.querySelectorAll("button").forEach((button) => {
        button.addEventListener("click", () => {
          const date = button.getAttribute("data-release-date");
          globeState.releaseFilter = date ? date : null;
          host.querySelectorAll("button").forEach((other) => other.classList.toggle("active", other === button));
          applyReleaseFilter();
        });
      });
    }
