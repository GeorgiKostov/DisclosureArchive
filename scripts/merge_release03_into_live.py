#!/usr/bin/env python3
"""Merge the Release 03 metadata-only export into the live (gh-pages) site.

We don't have R1/R2 raw data on this Mac, so we can't index R1+R2+R3 into a
single SQLite DB and run the normal export. Instead this script:
  ...
  3. Loads the R3-only export's data/documents.json (R3 records + R3 featured).
  4. Writes a merged data/documents.json with all docs and the new
     RELEASE_GROUPS (release-3, -2, -1).
  5. Drops every R3 records/<slug>-<id>.html into the live records/ directory.
  6. Appends R3 record URLs to sitemap.xml.
  7. Rewrites the JSON-LD itemListElement in index.html to add R3 highlights
     ALONGSIDE the existing R1/R2 ones, and updates the variableMeasured count.

Inputs (positional):
  live_dir       path to the live gh-pages checkout (read)
  r3_only_dir    path to public_site_r3only/ (read)
...
def main(live_dir: Path, r3_dir: Path, out_dir: Path) -> int:
    if out_dir.exists():
    ...
    r3 = load(r3_dir / "data" / "documents.json")

    # Use the new RELEASE_GROUPS from the R3 export (already has release-3, -2, -1).
    merged_groups = r3["release_groups"]
    print(f"release_groups: {[g['key'] for g in merged_groups]}")

    # Documents: union by doc_id (R3 wins on conflict, though there shouldn't be any).
    by_id = {d["doc_id"]: d for d in live["documents"]}
    r3_only_added = 0
    for d in r3["documents"]:
        if d["doc_id"] not in by_id:
            r3_only_added += 1
        by_id[d["doc_id"]] = d
    merged_docs = list(by_id.values())
    print(f"documents: live={len(live['documents'])} + R3-new={r3_only_added} = merged={len(merged_docs)}")

    # Featured: keep the live order, then append release-3 entries from R3 export.
    # Filter R3 featured to release-3 only so the accidental "Sary Shagan" R2
    # match in the R3-only export doesn't double-add an R2 highlight.
    live_featured = live.get("featured_documents", [])
    r3_featured = [f for f in r3.get("featured_documents", []) if f.get("release") == "release-3"]
    # Avoid duplicates by doc_id.
    seen = {f["doc_id"] for f in live_featured}
    merged_featured = list(live_featured) + [f for f in r3_featured if f["doc_id"] not in seen]
    print(f"featured: live={len(live_featured)} + R3-new={sum(1 for f in r3_featured if f['doc_id'] not in seen)} = merged={len(merged_featured)}")

    merged = {
        "schema": r3.get("schema") or live.get("schema"),
        "generated_at": r3.get("generated_at") or live.get("generated_at"),
    ...
    # --- copy R3 per-record HTML pages -------------------------------------- #
    r3_records = r3_dir / "records"
    live_records = out_dir / "records"
    r3_record_paths: List[str] = []
    copied = 0
    for src in sorted(r3_records.iterdir()):
        if not src.is_file() or not src.name.endswith(".html"):
            continue
        # Skip the records/ hub index file from the r3-only export; the live
        ...
        r3_record_paths.append(f"records/{src.name}")
        copied += 1
    print(f"copied {copied} R3 record HTML pages to {live_records.relative_to(out_dir.parent)}")

    # Regenerate the records/ hub from the R3 export so it covers all 298 docs,
    # not just the live R1+R2 set. The hub is small, deterministic, and built
    # from the merged JSON at runtime anyway; we just copy the R3-export hub.
    r3_hub = r3_records / "index.html"
    if r3_hub.exists():
        # The R3-only hub only lists 72 docs. We'd rather keep the live hub
    ...
    if sitemap_path.exists() and r3_record_paths:
        ...
        print(f"appended {len(r3_record_paths)} URLs to sitemap.xml")
    ...
    if index_path.exists():
        ...
        # 2. JSON-LD ItemList: append the R3 featured items to the highlights
        ...
        # Find the highlights itemListElement and inject R3 entries before its
        ...
        if marker in index_html:
            ...
            if arr_open != -1:
                ...
                if close_idx != -1:
                    ...
                    new_items = []
                    for offset, f in enumerate(r3_featured, start=1):
                    ...
                    print(f"injected {len(r3_featured)} R3 highlights into JSON-LD")
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("usage: merge_release03_into_live.py <live_dir> <r3_only_dir> <out_dir>", file=sys.stderr)
        raise SystemExit(2)
