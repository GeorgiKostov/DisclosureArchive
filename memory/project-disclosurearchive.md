...
- Public footer/legal notice update: the static export now renders copyright, contact, Legal / Impressum, privacy, security, sitemap, and source-code sections in the page footer. The contact email defaults to `contact@rebuilt.cards` and can be overridden with `DISCLOSURE_CONTACT_EMAIL` / `--contact-email`; the same address is written into structured data and `security.txt`.
